...
c.pocket48.newInfo = function(s){
    ...
    //groupId: groupName
    this.group=JSON.parse(JSON.stringify(s.content.group));
    this.team=JSON.parse(JSON.stringify(s.content.team));
    this.memberInfo=JSON.parse(JSON.stringify(s.content.memberInfo));
    this.period=JSON.parse(JSON.stringify(s.content.period));
    
    this.groupId2name = function(groupId){
        for (var i in this.group) {
            if (this.group[i].group_id==groupId) {
                return this.group[i].group_name
            }
        }
    };
    this.groupId2firstTeamId = function(groupId){
        for (var i in this.team) {
            if (this.team[i].team_id!=0 && this.team[i].group_id==groupId) {
    ...
    this.teamId2name = function(teamId, groupId = null){
        for (var i in this.team) {
            if (this.team[i].team_id==teamId && (this.team[i].group_id==groupId || groupId == null)) {
                return this.team[i].team_name;
            }
        }
    };
    this.teamId2color = function(teamId, groupId = null){
        for (var i in this.team) {
            if (this.team[i].team_id==teamId && (this.team[i].group_id==groupId || groupId == null)) {
                return this.team[i].color;
            }
        }
    };
    this.memberId2name = function(memberId){
        for (var i in this.memberInfo) {
            if (this.memberInfo[i].member_id == memberId) {
                return this.memberInfo[i].real_name;
            }
        }
    };
